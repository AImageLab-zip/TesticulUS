...
def parse_args(defaults: dict):
    ...
    # MODEL config
    parser.add_argument("--backbone", choices=[None, "uscl", "rn18", "rn50", "hico", "resnet_fpn"],
    ...
    parser.add_argument("--syn_data_path", type=str, required=True, help="Path to the synthetic data directory for ImageFolder")
...
def main():
    ...
    for seed in args.seeds:
        ...
        fake_dataset = ImageFolder(args.syn_data_path)
        real_dataset = USDataset(root_dir=args.dataset_dir, mode=args.dataset_mode, label_type=args.label_type)

        val_size = int(0.5 * len(real_dataset))
        test_size = len(real_dataset) - val_size
        generator = torch.Generator().manual_seed(seed)
        val_dataset, test_dataset = random_split(real_dataset, [val_size, test_size], generator=generator)

        if args.label_type == LabelTypes.OMO_DISOMO:
            if args.excluded_ids is not None:
                remove_bad_ids(real_dataset, args.excluded_ids)
            if args.flipped_ids is not None:
                flip_bad_ids(real_dataset, args.flipped_ids)
        ...
        metrics = tr.train_cv_fake(val_dataset, test_dataset, fake_dataset)
